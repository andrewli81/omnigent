...
async def _persist_native_compaction_item(
    ...
    *,
    ...
    Called when the cursor forwarder detects a compaction-completed
    summary blob. Queries the latest server-side item for
    ``last_item_id`` and reads the post-compaction messages from the
    cursor store to include as ``compacted_messages``.

    :param client: Omnigent HTTP client.
    :param session_id: Omnigent session/conversation id.
    :param store_path: Path to the cursor chat SQLite store.
    """
    ...
    # Read the post-compaction messages from the cursor store.
    compacted_messages = None
    try:
        rows = _read_blob_rows(store_path, 0)
        msgs = []
        for _rowid, _blob_id, raw_data in rows:
            # raw_data is a raw sqlite column (str or bytes); parse it.
        ...
        _logger.debug(
            "Failed to read cursor store for compaction persist",
            exc_info=True,
        )
                    ...
                    if await asyncio.to_thread(
                        ...
                        for item in items:
                            if item.item_type == "compaction_completed":
                                ...
                                # Persist a compaction boundary item so
                                # transcript rebuild from DB knows where
                                # compaction happened.
                                try:
